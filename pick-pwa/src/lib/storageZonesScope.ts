/**
 * storage_zones 的租戶範圍欄位：舊庫為 ，若已遷移為 company_id，
 * 請在環境變數設定：NEXT_PUBLIC_STORAGE_ZONES_SCOPE_COLUMN=company_id
 * （僅影響 storage_zones；label_records 等表仍可能使用  欄名）
 */
export type StorageZonesScopeColumn = "" | "company_id";

export function getStorageZonesScopeColumn(): StorageZonesScopeColumn {
  const raw = (
    process.env.NEXT_PUBLIC_STORAGE_ZONES_SCOPE_COLUMN ?? ""
  )
    .trim()
    .toLowerCase();
  return raw === "company_id" ? "company_id" : "";
}

/** 從列讀取租戶／公司識別碼（與 label QR 前綴一致） */
export function zoneRowScopeValue(
  row: { ?: unknown; company_id?: unknown },
): string {
  const col = getStorageZonesScopeColumn();
  if (col === "company_id") {
    return String(row.company_id ?? row. ?? "").trim();
  }
  return String(row. ?? row.company_id ?? "").trim();
}

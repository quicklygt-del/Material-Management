  ...
  return (
    <SupervisorOnlyGate>
      <nav className="sticky top-0 z-40 flex flex-wrap items-center justify-end gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur-sm">
        ...
        <Link
          href={appHref("/admin/warehouse-ledger")}
          className="text-sm font-black text-indigo-800 underline decoration-2"
        >
          倉儲總帳
        </Link>
        <Link href={appHref("/operator")} className="text-sm font-bold text-emerald-800 underline">
          倉管員工作台
        </Link>
        <Link href={appHref("/")} className="text-sm font-bold text-slate-600 underline">
          門戶首頁
        </Link>
      </nav>
      {children}

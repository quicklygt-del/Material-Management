...
export async function PATCH(
  req: Request,
  { params }: { params: { tenant_code: string } },
) {
  ...
  const { tenant_code: codeRaw } = params;
  const tenant_code = String(codeRaw ?? "").trim().padStart(3, "0");
  if (!/^\d{3}$/.test(tenant_code)) {
    return NextResponse.json({ error: "無效的 tenant_code" }, { status: 400 });
  }
